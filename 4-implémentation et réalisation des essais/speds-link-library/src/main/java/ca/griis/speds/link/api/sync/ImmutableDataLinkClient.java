 ...
 *
 * @brief @~french Implémentation de la classe ImmutableDataLinkClient.
 * @brief @~english Implementation of the ImmutableDataLinkClient class.
 */
 ...
 *
 * @brief @~french Offre les services d'un client de la couche liaison.
 * @par Details
 *      S.O.
 * @par Modèle
 *      S.O.
 ...
 *      S.O.
 */
public final class ImmutableDataLinkClient {
  private static final GriisLogger logger = getLogger(ImmutableDataLinkClient.class);

  private final ObjectMapper objectMapper;
  private final ConcurrentHashMap<UUID, UUID> trackingNumbers;

  ImmutableDataLinkClient(ObjectMapper objectMapper) {
   ...
   *        format JSON.
   * @return Les données à échanger au protocole.
   *
   * @par Tâches
   *      S.O.
   */
  public ProtocolIdu request(String idu) {
    logger.trace(Trace.ENTER_METHOD_1, "idu", idu);

    InterfaceDataUnit56Dto interfaceDataUnit56Dto = null;
    try {
      interfaceDataUnit56Dto =
          objectMapper.readValue(idu, InterfaceDataUnit56Dto.class);
    ...
    trackingNumbers.put(messageId, ici.getTrackingNumber());
   ...
   *      S.O.
   */
  public String confirm(ProtocolIdu protocolIdu) {
    logger.trace(Trace.ENTER_METHOD_1, "protocolIdu", protocolIdu);

    if (!trackingNumbers.containsKey(protocolIdu.messageIdentifier())) {
      throw new ProtocolException("Unable to find tracking number");
    }
    UUID trackingNumber = trackingNumbers.remove(protocolIdu.messageIdentifier());
