...
public class SesMsgRecHandler implements MessageHandler {
  ...
  public SesMsgRecHandler(ObjectMapper sharedObject, BlockingQueue<ExpandedSidu> queue,
      Map<SessionId, SessionInformation> sessionInformations) {
    logger.trace(Trace.ENTER_METHOD_3, "sharedObject", sharedObject, "queue", queue,
  ...
  public void handle(ExpandedSidu message) throws SilentIgnoreException {
    ...
    SesMsgRecDto sesMsgEnvDto;
    try {
      sesMsgEnvDto =
          sharedMapper.readValue((String) message.spdu().getContent(), SesMsgRecDto.class);
    } catch (JsonProcessingException e) {
      throw new SilentIgnoreException("Session required");
    }

    SessionId sessionId = new SessionId(sesMsgEnvDto.getSession());
