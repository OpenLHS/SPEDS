...
public class SesPubRecHandler implements MessageHandler {
  ...
  public SesPubRecHandler(ObjectMapper sharedObject, PgaService pgaService,
      BlockingQueue<ExpandedSidu> queue, Map<SessionId, SessionInformation> sessionInformations) {
    logger.trace(Trace.ENTER_METHOD_4, "sharedObject", sharedObject, "pgaService", pgaService,
  ...
  public void handle(ExpandedSidu message) throws SilentIgnoreException {
    ...
    // Session
    SessionInformation sessionInformation = validateSession(message);
    String peerKey = pgaService.getPublicKey(sessionInformation.pgaId, sessionInformation.peerId);
  ...
  private SessionInformation validateSession(ExpandedSidu message) throws SilentIgnoreException {
  ...
  private void dispatch(ExpandedSidu sidu) {
    try {
      queue.put(sidu);
