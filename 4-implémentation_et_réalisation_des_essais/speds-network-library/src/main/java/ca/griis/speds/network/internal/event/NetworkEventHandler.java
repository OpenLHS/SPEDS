 ...
 *
 * @brief @~french Implémentation de la classe NetworkEventHandler.
 * @brief @~english NetworkEventHandler class implementation.
 */

package ca.griis.speds.network.internal.event;
 ...
 *      S.O.
 */
public final class NetworkEventHandler implements HostEvent {
  private final AtomicReference<HostEvent> hostEventConsumer;

  public NetworkEventHandler() {
    this.hostEventConsumer = new AtomicReference<>();
  }

  public Boolean register(HostEvent subscriber) {
    return hostEventConsumer.compareAndSet(null, subscriber);
  }

  @Override
  public void notifyIdu(String idu) {
    HostEvent current = hostEventConsumer.get();
    if (current == null) {
