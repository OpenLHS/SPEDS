...
public class SesFinRecHandler implements MessageHandler {
  ...
  public SesFinRecHandler(HostStartupContext ctx, InitiatorEvent initiatorEvent) {
  ...
  public Optional<String> handle(ExpandedSidu message) {
    ...
    try {
      ...
      final SesFinRecDto sesFinRecDto =
          mapper.readValue((String) message.spdu().getContent(), SesFinRecDto.class);

      sessionId = new SessionId(sesFinRecDto.getSession());
      ...
      if (validatedSession.isPresent()) {
        ...
        if (validatedSession.get().peerId().equals(code)) {
          ...
          if (verifyStamp) {
            final Boolean verifyToken = sesFinRecDto.getToken().equals(sessionInformation.token());
            if (verifyToken) {
              transportHost.response(entityCode, sessionId, message, "SUCCEED");

              initiatorEvent.notifyFinRec(message);
            } else {
